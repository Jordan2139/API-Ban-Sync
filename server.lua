                ...
                if string.match(id, "discord:") then
                    ...
                    PerformHttpRequest(string.format(config.apiURL .. '' .. discordID), function (err, res)
                        ...
                        if(data["status"] == 'banned') then
                            ...
                            sendToDisc("[API Ban Sync] Banned Member Attempted Connection!", '' .. GetPlayerName(src) .. " was declined connection due to them being banned \nSteam: **" .. steam .. '**\nDiscord Tag: ** <@' .. ids.discord:gsub('discord:', '') .. '> **\nDiscord UID: **' .. ids.discord:gsub('discord:', '') .. '**\nIP:** ||'.. ids.ip:gsub('ip:', '').. '||**');
                            end
                            deferrals.done("You are banned from " .. config.APIName .. " and can't join the server.")
                            CancelEvent()
                        else
                            if config.Discord.Enabled then
                            sendToDiscGood("[API Ban Sync] Member Connecting...", '' .. GetPlayerName(src) .. " was allowed connection due to them not being banned\nSteam: **" .. steam .. '**\nDiscord Tag: ** <@' .. ids.discord:gsub('discord:', '') .. '> **\nDiscord UID: **' .. ids.discord:gsub('discord:', '') .. '**\nIP:** ||'.. ids.ip:gsub('ip:', '').. '||**');
                            end
    ...
    embed = {
        {
            ...
            ["description"] = "" .. message ..  "",
            ["footer"] = {
                ["text"] = '[API Ban Sync] Created By: Jordan.#2139',
            },
        }
    }
    PerformHttpRequest(config.Discord.WebHookURL, 
